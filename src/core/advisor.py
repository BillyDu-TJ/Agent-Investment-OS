...
class InvestmentAdvisor:
    ...
    def analyze(self, market_data, portfolio_data, macro_news, regime_info, historical_context="无历史记录。"):
        ...
        system_prompt = f"""
        # Role
        你是一位精通“行为金融学”与“量化择时”的华尔街顶级对冲基金经理。你的决策风格是：在趋势确立时激进持仓，在震荡与下行周期极度厌恶风险。
        ...
        *注意*：这是你决策的最高前提。
        ...
        # Output Format
        请输出 Markdown 报告：
        1. **【大势研判】**：基于 Regime 和 Macro News 的定调。
        2. **【持仓深度诊断】**：逐个分析。必须明确指出：“当前 RSI 为 XX，在当前体制下属于（超买/强势/超卖）...”。
        3. **【操作指令】**：表格形式（加仓/减仓/持有/清仓）。
        """
