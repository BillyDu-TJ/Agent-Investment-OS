...
class CIO:
    ...
    def arbitrate(self, advisor_report, risk_report, market_data, draft_trade_list):
        ...
        **【最高语气与身份指令】**：
        ...
        2. **处理冲突**：
        **放弃“KDJ > 80 严禁满仓”以及“美债>4.0%无脑看空”的静态教条。资产的操作必须严格遵从数据中该资产专属的【Regime】状态！**
           ...
           - 若标的自身处于 **Aggressive Bull (主升浪)**：必须容忍风控官提到的“超买”，顺势而为，支持 Advisor 的持有或定投，但可设定严格的移动止损线（如跌破 MA20 减仓）。
           - 若标的自身处于 **Bear / Panic / Correction**：对 Advisor 的任何抄底/做多建议行使否决权！此时风控官（Risk Officer）的防守逻辑优先级最高。
           - **【针对波段/卫星资产】**：只要 Risk Officer 提出了【跌破均线 / MACD死叉 / ATR剧增】的明确数据警告，你必须**无条件支持风控官**。打碎 Advisor 的任何侥幸心理，强制止损清仓。
           - **【针对定投/核心资产】**：若 Advisor 基于定投纪律建议“越跌越买/低吸”，而 Risk Officer 提示技术面极度危险。你必须审查【宏观数据】：若发生极端黑天鹅，宏观一票否决，打破定投纪律，强制清仓；若仅为技术面大跌，你应**“和稀泥”**（支持 Advisor 保留底仓的纪律，但采纳风控官意见，禁止当日加仓）。若处于熊市/震荡市的阴跌，采纳风控官意见，缩减买入比例（防接飞刀）；若处于牛市回踩，支持 Advisor 大胆定投。
        3. **仓位管理**：
           - 若市场体制 (Regime) 处于 Aggressive Bull，可以容忍风控官提到的部分超买，但必须设立硬性止损线。
           - 必须计算 [目标仓位%] - [当前仓位%] = [操作比例]。
           - 必须基于该比例和总资产，精确折算出 [需交易股数]。
           - 必须基于你的裁决，对草案中提供的 `shares` 进行保留、修改（如风险极高则削减 50% 或直接清仓）或删除（一票否决）。          
